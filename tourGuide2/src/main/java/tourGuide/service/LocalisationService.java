...
import tourGuide.webClient.gpsUtil.Attraction;
import tourGuide.webClient.gpsUtil.GpsUtil;
import tourGuide.webClient.gpsUtil.Location;
        ...
        for (double key : keys) {
            if (i < 5) {
                ...
                NearbyAttraction nearbyAttraction = new NearbyAttraction(attraction.getLatitude(),
                        ...
                        rewardsCentral.getAttractionRewardPoints(attraction.getAttractionId(), user.getUserId()));
...
//            .flatMap(attraction->{
//                return Flux.just(attractionsMap.put(getDistance(visitedLocation.getLocation(), attraction), attraction));
//                    })
//            .flatMap(attraction -> {for (int i=0; i<5; i++){return Flux.just(attraction, attraction);}})
//            .collectList();
